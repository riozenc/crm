...
public class KickoutSessionFilter extends AdviceFilter {
	 ...
	 * during filter chain execution. That is, if {@link #executeChain
	 * executeChain} throws an exception, this method will never be called. Be
	 * aware of this when implementing logic. Most resource 'cleanup' behavior
	 * is often done in the
	 ...
	 * preHandle} returns {@code false} or if an exception is thrown during
	 * filter chain processing. Can be used for resource cleanup if so desired.
	 * <p/>
	 * The default implementation does nothing (no-op) and exists as a template
	 * method for subclasses.
	 *
	 ...
	 *            the outgoing ServletResponse
	 * @param exception
	 *            any exception thrown during {@link #preHandle preHandle},
	 ...
	 * @param existing
	 *            any exception that might have occurred while executing the
	 *            {@code FilterChain} or pre or post advice, or {@code null} if
	 *            the pre/chain/post execution did not throw an
	 *            {@code Exception}.
	 * @throws ServletException
	 *             if any exception other than an {@code IOException} is thrown.
	 * @throws IOException
	 *             if the pre/chain/post execution throw an {@code IOException}
	 */
